...
contract('SupplyChain', function(accounts) {
    ...
    const productPrice = web3.utils.toWei('1', "ether") //web3.utils.toWei(number [, unit])
    var itemState = 0
    ...
    it("Testing smart contract function sellItem() that allows a farmer to sell coffee", async() => {
        ...
        const resultBufferTwo = await supplyChain.fetchItemBufferTwo.call(upc);
        // Verify the result set
        assert.equal(resultBufferTwo[4], productPrice, 'Invalid item productPrice');
    ...
    it("Testing smart contract function fetchItemBufferTwo() that allows anyone to fetch item details from blockchain", async() => {
        ...
        assert.equal(resultBufferTwo[2], productID, 'Invalid item productID');
        assert.equal(resultBufferTwo[3], productNotes, 'Invalid item productNotes');
        assert.equal(resultBufferTwo[4], productPrice, 'Invalid item productPrice');
        assert.equal(resultBufferTwo[5], 7, 'Invalid item State');
        assert.equal(resultBufferTwo[6], distributorID, 'Invalid item distributorID');
        assert.equal(resultBufferTwo[7], retailerID, 'Invalid item retailerID');
        assert.equal(resultBufferTwo[8], consumerID, 'Invalid item consumerID');
    })
